...
const space = [];
// generate a space that can hold 6 iterations
...
const countActiveNeighbours = (x, y, z) => {
  let count = 0;
...
input.split('\n').forEach((j, jIndex) => {
  j.split('').forEach((i, iIndex) => {
    space[midZ][jIndex + MAX_CYCLES][iIndex + MAX_CYCLES] = i;
  });
});
...
for (let cycle = 0; cycle < MAX_CYCLES; cycle += 1) {
  ...
  iterSpace((x, y, z) => {
    const activeNeighbours = countActiveNeighbours(x, y, z);
    const currentNode = space[z][y][x];

    if (currentNode === '#' && ![2, 3].includes(activeNeighbours)) {
      invertList.push({ x, y, z });
  ...
  invertList.forEach((invItem) => {
    const sItem = space[invItem.z][invItem.y][invItem.x];
    space[invItem.z][invItem.y][invItem.x] = sItem === '#' ? '.' : '#';
  });
